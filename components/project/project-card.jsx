...
// app/projects/components/project-card.jsx
      ...
      <Card className="flex flex-col justify-between hover:shadow-lg transition-shadow duration-300">
        <CardHeader className="flex-row items-start justify-between">
          <div>
            ...
            <CardDescription className="font-mono pt-1 text-xs">{project.db_schema_name}</CardDescription>
          </div>
          <AlertDialog><CardAction>
              <DropdownMenu>
        ...
        <CardFooter>
          <Button variant="outline" className="w-full" onClick={handleCardClick}>
                         select
                      </Button>
        </CardFooter>
      </Card>
...
export function NewProjectCard({ onSuccess }) {
    ...
    return (
        <>
            ...
            <CreateProjectDialog open={open} onOpenChange={setOpen} onSuccess={onSuccess} />
        </>
    );
}
